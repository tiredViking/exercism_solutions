...
def check_drinks(drink_name, drink_ingredients):
    ...
    string = "Mocktail" if set(drink_ingredients).isdisjoint(ALCOHOLS) else "Cocktail"
    return f"{drink_name} {string}"
...
def tag_special_ingredients(dish):
    ...
    list_of_ingredients = dish[1]
    tagged_ingredients = list()

    for item in list_of_ingredients:
        if item in SPECIAL_INGREDIENTS:
            tagged_ingredients.append(item)
        else: 
            continue
    result = (dish[0], set(tagged_ingredients))
    return result
...
def singleton_ingredients(dishes, intersection):
    ...
    result = set()
    for item in dishes:
        result = result.union(item.difference(intersection))
